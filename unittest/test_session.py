...
def test_data_request__get__ok(vit_requests_mock):
    mock_response = MockResponse(200)
    # https://docs.python.org/3/library/unittest.mock-examples.html#mocking-chained-calls
    vit_requests_mock.Session.return_value.request.return_value = mock_response
    ...
    def mock_request_effect(**kwargs):
        if kwargs["method"] == "POST":
            return MockResponse(200,
                                content=json.dumps({"access_token": "a", "token_type": "b", "expires_in": 10}).encode())
...
@patch('volue_insight_timeseries.session.auth', MagicMock())
@patch('volue_insight_timeseries.session.requests')
def test_send_data_request__long_url__correct(vit_requests_mock, urlbase, url, longurl_expected):
    vit_requests_mock.Session.return_value.request.return_value = MockResponse(200)

    session = make_vit_session()
    session.data_request('GET', urlbase=urlbase, url=url)

    call_args = vit_requests_mock.Session.return_value.request.call_args
...
@patch('volue_insight_timeseries.session.auth', MagicMock())
@patch('volue_insight_timeseries.session.requests')
def test_send_data_request__databytes__correct(vit_requests_mock, data, rawdata, databytes_expected):
    vit_requests_mock.Session.return_value.request.return_value = MockResponse(200)
...
@patch('volue_insight_timeseries.session.auth', MagicMock())
@patch('volue_insight_timeseries.session.requests')
def test_send_data_request__retries__correct(vit_requests_mock, failed_status_code):
    vit_requests_mock.Session.return_value.request.return_value = MockResponse(failed_status_code)
    ...
    assert vit_requests_mock.Session.return_value.request.call_count == retries_count + 1


@patch('volue_insight_timeseries.session.auth')
@patch('volue_insight_timeseries.session.requests')
def test_data_request__get_auth__first_failed_then_ok(vit_requests_mock, auth_mock):
    mock_response = MockResponse(200)
    vit_requests_mock.Session.return_value.request.return_value = mock_response
    oauth_mock = Mock()
    ...
    assert vit_requests_mock.Session.return_value.request.call_count == 1
    call_args = vit_requests_mock.Session.return_value.request.call_args
...
@patch('volue_insight_timeseries.session.auth')
@patch('volue_insight_timeseries.session.requests')
def test_data_request__fail_too_many_times(vit_requests_mock, auth_mock):
    mock_response = MockResponse(200)
    vit_requests_mock.Session.return_value.request.return_value = mock_response
    oauth_mock = Mock()
    auth_mock.OAuth.return_value = oauth_mock
    ...
    assert vit_requests_mock.Session.return_value.request.call_count == 0
